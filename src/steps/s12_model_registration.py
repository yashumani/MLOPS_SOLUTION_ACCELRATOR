...
class ModelRegistry:
    ...
    def register_champion_model(
        ...
        try:
            ...
            # Log model artifact to current MLflow run
            ...
            if run_id:
                ...
                # Fallback: Register model from artifact URI
                model_uri = f"file://{model_file}"
...
def main():
    ...
    # Log to MLflow (wrap artifact logging to handle azureml:// URI issue)
    try:
        mlflow.log_params({
            "model_name": registry_info["model_name"],
            "model_version": registry_info["version"],
            "model_stage": registry_info["stage"]
        })
        logger.info("✅ MLflow params logged")
    except Exception as e:
