...
def main():
    ...
    df_full = pd.read_csv(args.dataset_in)
    ...
    try:
            ...
            try:
                ...
                for engine in engines:
                    ...
                    try:
                        ...
                        # NOTE: timed_out check removed — a timed-out run that
                        # produced a valid model should still be a champion
                        # candidate.  The leaderboard fallback path already
                        # includes timed-out results, so the tracking loop must
                        # match to avoid champion-mismatch errors.
                        if model is not None and not result.failed:
        ...
        if champion_key != best_key:
            ...
            raise RuntimeError(f"Phase B aborted: {diagnostic}")
        elif best_model is None:
            print(f"   ⚠️  WARNING: No champion model available (all attempts failed/timed out)")
            print(f"   ⚠️  Skipping model.pkl creation")
