...
def eval_model(model, X_test, y_test, task: str, label_encoder=None, threshold=None):
    ...
    try:
        if task == "classification":
            ...
            if n_clusters > 1:
                try:
                    metrics["silhouette_score"] = float(silhouette_score(X_test, preds))
                except Exception as e:
                    print(f"  ⚠️  Could not compute silhouette_score: {e}")
                    metrics["silhouette_score"] = -1.0
                
                try:
                    metrics["davies_bouldin_score"] = float(davies_bouldin_score(X_test, preds))
                except Exception as e:
                    print(f"  ⚠️  Could not compute davies_bouldin_score: {e}")
                    metrics["davies_bouldin_score"] = float('inf')
                
                try:
                    metrics["calinski_harabasz_score"] = float(calinski_harabasz_score(X_test, preds))
                except Exception as e:
    ...
    try:
        ...
        if report["baseline_metrics"]:
            for key, val in report["baseline_metrics"].items():
                try:
                    logger.log_metric(f"baseline_{key}", float(val))
        ...
        if report["phaseb_metrics"]:
            for key, val in report["phaseb_metrics"].items():
                try:
                    logger.log_metric(f"phaseb_{key}", float(val))
                except (ValueError, TypeError):
                    # Skip non-numeric values
                    pass
        ...
        if report["phasec_metrics"]:
            for key, val in report["phasec_metrics"].items():
                try:
                    logger.log_metric(f"phasec_{key}", float(val))
                except (ValueError, TypeError):
                    # Skip non-numeric values
                    pass
        
        # Log champion selection
        selection = report.get("selection", {})
        logger.log_param("champion_phase", selection.get("key", "none"))
        if selection.get("score") is not None:
            logger.log_metric("champion_score", float(selection["score"]))
        
        # Log primary metric for comparison (task-aware)
        if report.get("baseline_metrics") and primary_metric in report["baseline_metrics"]:
            logger.log_metric("final_baseline_metric", report["baseline_metrics"][primary_metric])
        if report.get("phaseb_metrics") and primary_metric in report["phaseb_metrics"]:
            logger.log_metric("final_phaseb_metric", report["phaseb_metrics"][primary_metric])
        if report.get("phasec_metrics") and primary_metric in report["phasec_metrics"]:
            logger.log_metric("final_phasec_metric", report["phasec_metrics"][primary_metric])
...
def main():
    ...
    with open(all_metrics_path, "w") as f:
        json.dump(all_stage_metrics, f, indent=2)
    ...
    try:
        ...
        if champion_model_obj is not None and task_type != "clustering":
            ...
            with open(shap_path, "w") as sf:
                json.dump(shap_summary, sf, indent=2)
    ...
    if _variant_rankings_list:
        ...
        if _all_variant_results:
            ...
            with open(_avr_path, "w") as _avrf:
                json.dump(_all_variant_results, _avrf, indent=2)
